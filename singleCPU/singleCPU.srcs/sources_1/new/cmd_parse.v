...
module cmd_parse(
	...
	localparam// States for the main state machine
	 IDLE      = 3'b000,
	 CMD_WAIT  = 3'b001,
	 GET_ARG   = 3'b010,
	 SEND_RESP = 3'b011;
	...
	// Code
	//***************************************************************************
	always @(posedge clk_rx)// capture the rx_data_rdy for edge detection
	begin
		if (rst_clk_rx)
		begin
	...
	always @(posedge clk_rx)// Echo the incoming character to the output, if there is room in the FIFO
	begin
		if (rst_clk_rx)
		begin
			send_char_val <= 1'b0;
	...
	begin
		...
		begin// Defaults - overridden in the appropriate state
			case (state)
				IDLE:
				begin // Wait for the '*'
				...
				CMD_WAIT:
				begin // Validate the incoming command
					if (new_char)
					begin
						cur_cmd <= rx_data[7:0];
						case (rx_data[6:0])
							CMD_W, CMD_w_l:
							begin// Get 8 characters of arguments
								state   <= GET_ARG;
								arg_cnt <= 3'd7;
							end  // Ww
							CMD_N, CMD_n_l:
							begin // Get 4 characters of arguments
								state   <= GET_ARG;
								arg_cnt <= 3'd3;
							end  //  Nn
							CMD_F, CMD_f_l:
							begin // Get 1 characters of arguments
								state   <= GET_ARG;
								arg_cnt <= 3'd0;
							end  //  Ff
							default:
				...
				begin
					...
					begin
						...
						begin
							...
							begin// append the current digit to the saved ones
								arg_sav <= arg_val;
								arg_cnt <= arg_cnt - 1'b1; // Wait for the next character
							...
							begin
								case (cur_cmd) 
									...
									begin
										case(arg_val[3:0])
											4'h2: bt_data32 <= 32'hAAAFABA1;
											4'h3: bt_data32 <= 32'hAAAFABA2;
											4'h4: bt_data32 <= 32'hAAAFABA3;
											4'h5: bt_data32 <= 32'hAAAFABA5;
											4'h6: bt_data32 <= 32'hAAAFABA8;
											4'h7: bt_data32 <= 32'hAAFABA13;
											4'h8: bt_data32 <= 32'hAAFABA21;
											4'h9: bt_data32 <= 32'hAAFABA34;
											4'ha: bt_data32 <= 32'hAAFABA55;
											4'hb: bt_data32 <= 32'hAAFABA89;
											4'hc: bt_data32 <= 32'hAFABA144;
											4'hd: bt_data32 <= 32'hAFABA233;
											4'he: bt_data32 <= 32'hAFABA377;
											4'hf: bt_data32 <= 32'hAFABA610;
											default: bt_data32 <= 32'hEEEEEEEE;
										endcase
										bt_data16      <= {8'b0, arg_val[3:0]};
										send_resp_val  <= 1'b1;// Send OK
										send_resp_type <= RESP_OK;
										state          <= SEND_RESP;
									end // Ff
